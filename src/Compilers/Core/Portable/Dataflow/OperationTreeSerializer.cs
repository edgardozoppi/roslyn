...
{
    internal sealed class OperationTreeSerializer : OperationWalker
    {
        ...
        public OperationTreeSerializer(IOperation root, int initialIndent)
        {
            _root = root;
        ...
        {
            var walker = new OperationTreeSerializer(operation, 0);
        ...
        public override void VisitConditionalGotoStatement(IConditionalGotoStatement operation)
        {
            ...
            LogSymbol(operation.Target, " (Label Symbol");
        ...
        public override void VisitSequenceExpression(ISequenceExpression operation)
        {
            ...
            var expressionsStr = $"{operation.Expressions.Length} expressions";
            var localStr = !operation.Locals.IsEmpty ? $", {operation.Locals.Length} locals" : string.Empty;
            LogString($" ({expressionsStr}{localStr})");
            LogCommonPropertiesAndNewLine(operation);
